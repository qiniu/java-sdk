...
public class ResumeableDemo {
	...
	public void testFileNoResume() throws AuthException, JSONException {
		...
		RandomAccessFileUpload up = new RandomAccessFileUpload(new File(file), authorizer, key, null);
		...
		System.out.println((e-s+1) + "  " + ((up.getCurrentUploadLength()/1024)/((e-s+1)/1000)));
	...
	public void testFileFileResume() throws AuthException, JSONException {
		...
		FileResume.RESUME_FILENAME_TYPE = FileResume.MD5_TYPE; // 默认 FileResume.BASE64_TYPE
		...
		System.out.println(ret.response);
		System.out.println(ret.statusCode);
		System.out.println((e-s+1) + "ms,  " + ((up.getCurrentUploadLength()/1024)/((e-s+1)/1000)) + "KB/s");
	}
	
	
	//@Test
	public void testStream() throws AuthException, JSONException, FileNotFoundException {
		String key = uuid + "____" + file;
		PutPolicy putPolicy = new PutPolicy(bucket);
		String token = putPolicy.token(mac);
		...
		StreamSliceUpload up = new StreamSliceUpload(new FileInputStream(f), authorizer, key, null, f.length());
		up.httpClient = Http.getClient();
	...
	private void handle(final SliceUpload up){
		Thread t = new Thread(){
			public void run(){
				...
				while(true){
				try {
					String msg = up.getContentLength() + " :  " + up.getCurrentUploadLength() + " -- " + up.getLastUploadLength() ;
					if(up.getContentLength() > 0){
						msg += "  :  " + (up.getCurrentUploadLength() + up.getLastUploadLength()) * 100 / up.getContentLength() + "%";
					}
					System.out.println(msg);
					long e = System.currentTimeMillis();
					System.out.println(Thread.currentThread().getId() + "  " + (e-s+1) + "ms,  " + ((up.getCurrentUploadLength()/1024)/((e-s+1)/1000)) + "KB/s");
					Thread.sleep(1 * 1000);
				} catch (Exception e) {
