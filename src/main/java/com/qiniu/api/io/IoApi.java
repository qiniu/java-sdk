...
public class IoApi {
	...
	private static PutRet put(String uptoken, String key,
			AbstractContentBody contentBody, PutExtra extra) {
		...
		try {
			...
			requestEntity.addPart("file", wrapAbstractContentBody(contentBody, extra.mimeType));
			if (isAllowedScope(key)) {
	...
	private static AbstractContentBody wrapAbstractContentBody(final AbstractContentBody contentBody, String mimeType){
		return mimeType == null || mimeType.trim().isEmpty() ? contentBody :
				new AbstractContentBody(mimeType){
		
					@Override
					public String getFilename() {
						return contentBody.getFilename();
					}
					...
					public String getCharset() {
						return contentBody.getCharset();
					}
		
					@Override
					public String getTransferEncoding() {
						return contentBody.getTransferEncoding();
					}
		
					@Override
					public long getContentLength() {
						return contentBody.getContentLength();
					}
	...
	private static boolean isAllowedScope(String key) {
	...
	public static PutRet put(String uptoken, String key, InputStream reader,
		...
		String fileName = isAllowedScope(key) ? key : "?";
