...
public class AppManager {
     ...
     * @param noAutoCloseRoom bool 类型，可选，禁止自动关闭房间。默认为 false ，即用户退出房间后，房间会被主动清理释放
     * @param noAutoCreateRoom bool 类型，可选，禁止自动创建房间。默认为 false ，即不需要主动调用接口创建即可加入房间。
     * @param noAutoKickUser bool 类型，可选，禁止自动踢人（抢流）。默认为 false ，即同一个身份的 client (app/room/user) ，新的连麦请求可以成功，旧连接被关闭。
     * @return
     * @throws QiniuException
     */
    public String creatApp(String hub,String title,int maxUsers,boolean noAutoCloseRoom,boolean noAutoCreateRoom,boolean noAutoKickUser) throws QiniuException{
        if(hub!=null){
            params.put("hub",hub);
        }
        if(title!=null){
        ...
        params.put("noAutoCreateRoom",noAutoCreateRoom);
     ...
     * @throws QiniuException
     */
    public String updateApp(String appId,String hub,String title,int maxUsers,boolean noAutoCloseRoom,boolean noAutoCreateRoom,boolean noAutoKickUser) throws QiniuException{
        if(hub!=null){
            params.put("hub",hub);
        }
        if(title!=null){
